...
export default function Hero() {
  ...
  return (
    ...
    >
      {/* ── Wordmark layer ──────────────────────────────────────────────── */}
      <motion.div
        style={prefersReducedMotion ? undefined : { y: typeY, opacity: typeOpacity }}
        className="order-2 mt-8 flex flex-col items-start gap-2 lg:absolute lg:inset-x-gutter lg:top-[42%] lg:z-10 lg:mt-0 lg:flex-row lg:items-baseline lg:justify-between lg:gap-6"
      >
        ...
        <motion.span
          ...
          initial="hidden"
          animate="visible"
          className="flex font-display text-display-xl uppercase leading-none text-plum"
        >
          <span className="sr-only">{site.wordmark.solid}</span>
          {Array.from(site.wordmark.solid).map((letter, index) => (
            <motion.span key={`${letter}-${index}`} aria-hidden variants={letterIn} className="inline-block">
              {letter}
            </motion.span>
          ))}
        </motion.span>
        ...
        >
          <TypingName
            ...
            className="font-display text-display-xl uppercase leading-none text-outline"
          />
          ...
          <motion.ul
            ...
            initial="hidden"
            animate="visible"
            className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-1 text-eyebrow uppercase text-plum-muted lg:justify-end"
          >
            {site.roles.map((role, index) => (
              <motion.li key={role} variants={letterIn} className="flex items-center gap-3">
      ...
      <div className="order-1 flex justify-center lg:flex lg:min-h-[100svh] lg:items-end">
        <HeroImage src={site.images.hero} alt={`${site.name}, ${site.tagline}`} />
      </div>
      ...
      >
        <a href="#projects" className="group flex items-center gap-3 transition-colors hover:text-plum">
